...
function save(getArgs, filepath, text, callback) {
  ...
  async.eachSeries(text_parts, function(part, cb) {
    var pinky = text_parts.pinkyOf(part);
    var headers = getHeader();
    var args = getArgs(part, pinky, total);
...
function stream(getArgs, text) {
  ...
  return MultiStream(text_parts.map(function(part, pinky) {
    var headers = getHeader();
    var args = getArgs(part, pinky, total);
    var fullUrl = GOOGLE_TTS_URL + args
...
function getArgsFactory(lang){
  return function (text, pinky, total) {
    var textlen = text.length;
    var encodedText = encodeURIComponent(text);
    var language = lang || 'en';
    return `?ie=UTF-8&tl=${language}&q=${encodedText}&total=${total}&idx=${pinky}&nuy=tw-ob&textlen=${textlen}`
  }
}
