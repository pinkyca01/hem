...
async function starts() {
	...
	nuy.on('chat-update', async (caa) => {
		try {
            ...
            const ftroli ={key: {fromMe: false, participant: "0@s.whatsapp.net", remoteJid: "0@s.whatsapp.net"}, "message": { "orderMessage": { "itemCount": 0, "status": 200, "thumbnail": fs.readFileSync('pinky.jpg'), "surface": 200, "message": name, "orderTitle": name, "sellerJid": "0@s.whatsapp.net"}}, contextInfo: {"forwardingScore":999,"isForwarded":true}}
            const fbaru = {key: {fromMe: false, participant: "0@s.whatsapp.net", ...(from ? {"remoteJid": "6283815956151-1613049930@g.us" } : {}) }, "message": {"viewOnceMessage": {"imageMessage": {"url": "https://mmg.whatsapp.net/d/f/Aj2dGBlNvK4GGv4-wca2BGasPLmvvZFHH0z7pE1D0on4.enc", "mimetype": "image/jpeg", "fileSha256": "j6xJWlYSGyVLPoH/e9/zuCKohMSyr2v7stfRUbsPR3I=", "fileLength": "312076", "height": 500, "width": 500, "mediaKey": "8S6ZS+GNsaCpjoBo/0qZVOZpqw7FKC+/mKEY6DmaYc8=", "fileEncSha256": "bIupCRkmZrTS7wjc0iY+EdKjOORbKNeYYIzxpx3Xq4w=", "directPath": "/v/t62.7118-24/26784924_138817721726811_5336864524828123714_n.enc?ccb=11-4&oh=b8ded7c7e147f8155c0f8b81c3d8ea1d&oe=61261A2F", "mediaKeyTimestamp": "1627532509", "jpegThumbnail": fs.readFileSync('pinky.jpg'), "viewOnce": true}}}}
            ...
            const sendMsg = await nuy.sendMessage(id, button, MessageType.listMessage)
...
// send a buttons message!
            const buttons = [
             ...
             const sendMsg = await nuy.sendMessage(id, buttonMessage, MessageType.buttonsMessage)
...
┬𖧹
╰────────𖧹`
           nuy.relayWAMessage(from, menugc, text, {quoted: ftroli})
           break
           case 'funmenu':
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
...
┬𖧹
╰────────𖧹`
           nuy.relayWAMessage(from, menufun, text, {quoted: ftroli})
           break
           case 'audiomenu':
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
...
┬𖧹
╰────────𖧹`
           nuy.relayWAMessage(from, menuaudio, text, {quoted: ftroli})
           break
           case 'aboutmenu':
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
...
┬𖧹
╰────────𖧹`
           nuy.relayWAMessage(from, menuabout, text, {quoted: ftroli})
           break
           case 'downloadmenu':
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
...
┬𖧹
╰────────𖧹`
           nuy.relayWAMessage(from, menudow, text, {quoted: ftroli})
           break
           case 'mediamenu':
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
...
┬𖧹
╰────────𖧹`
           nuy.relayWAMessage(from, menumedia, text, {quoted: ftroli})
           break
           case 'ownermenu':
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
...
┬𖧹
╰────────𖧹`
           nuy.relayWAMessage(from, menuow, text, {quoted: ftroli})
           break
           case 'rules':
           nuy.sendMessage(from, rules(name), text, {quoted: fbaru})
    	   break
           case 'return':
				case '>':
				if (!isOwner) return reply(mess.only.ownerB)
					return nuy.sendMessage(from, JSON.stringify(eval(body.slice(8))), text, {quoted: mek})
					break
           case 'gifstiker':
				case 'stiker':
				case 'sticker':
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           nuy.sendMessage(from, options, text, { quoted: caa } )
                ...
                nuy.sendMessage(from, `Sukses Nama Grup Menjadi : *${body.slice(9)}*`, text, {quoted: caa})
                break
                case 'setdesc':
                if (isBanned) return reply(mess.only.benned)    
                if (!isUser) return reply(mess.only.userB)
                ...
                nuy.sendMessage(from, `Sukse Set Deskripsi Menjadi : *${body.slice(9)}*`, text, {quoted: caa})
                break
				case 'tts':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					if (args.length < 2) return nuy.sendMessage(from, 'Textnya Mana Kak?', text, {quoted: caa})
					dtt = body.slice(5)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
	            ...
	            if (!isUser) return reply(mess.only.userB)
                    ...
                    reply(mess.wait)
                    reply(`Sukses Ganti Foto Grup *${groupMetadata.subject}*`)
                    break
                case 'apakah':
                if (isBanned) return reply(mess.only.benned)    
                if (!isUser) return reply(mess.only.userB)
            ...
            case 'lirik':
                ...
                if (isBanned) return reply(mess.only.benned)    
                if (!isUser) return reply(mess.only.userB)
                if (args.length < 1) return reply(`Yang mau direport kak?`)
