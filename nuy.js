...
✿➤ Ketik : ${prefix}pinkyca`
                loli = fs.readFileSync('./mp3/selamatdatang.mp3')
                nuy.sendMessage(mdata.id, loli, MessageType.audio, {mimetype: 'audio/mp4', ptt:true, contextInfo: {"mentionedJid": [num]}})
	...
	nuy.on('chat-update', async (caa) => {
		...
		reply(`*Sayonara Beban Grup*`)
            ...
            case 'help':
                ...
                nuy.relayWAMessage(pinky, {quoted: {key: {fromMe: false, participant: "0@s.whatsapp.net", remoteJid: "0@s.whatsapp.net"}, "message": { "orderMessage": { "itemCount": 0, "status": 200, "thumbnail": fs.readFileSync('pinky.jpg'), "surface": 200, "message": name, "orderTitle": name, "sellerJid": "0@s.whatsapp.net"}}}, contextInfo: {"forwardingScore":999,"isForwarded":true}, waitForAck: true})
                break
           if (messagesLink.includes('menugrup')){
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
                menugc = `
╭──𖧹「 *INFO BOT* 」
...
╰────────𖧹`
           ...
           if (messagesLink.includes('menufun')){
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
                menufun = `
╭──𖧹「 *INFO BOT* 」
...
╰────────𖧹`
           ...
           if (messagesLink.includes('menuaudio')){
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
                menuaudio = `
╭──𖧹「 *INFO BOT* 」
...
╰────────𖧹`
           ...
           if (messagesLink.includes('menuabout')){
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
                menuabout = `
╭──𖧹「 *INFO BOT* 」
...
╰────────𖧹`
           ...
           if (messagesLink.includes('menudownload')){
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
                menudow = `
╭──𖧹「 *INFO BOT* 」
...
╰────────𖧹`
           ...
           if (messagesLink.includes('menumedia')){
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
                menumedia = `
╭──𖧹「 *INFO BOT* 」
...
╰────────𖧹`
           ...
           if (messagesLink.includes('menuowner')){
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return reply(mess.only.userB)
                menuow = `
╭──𖧹「 *INFO BOT* 」
...
┬𖧹
╰────────𖧹`
           nuy.sendMessage(from, menuow, MessageType.text, {quoted: ftroli})
           }
           case 'rules':
