...
async function starts() {
	...
	nuy.on('chat-update', async (caa) => {
		...
		if (!isGroup) return
		if (!isBadWord) return
		if (isGroupAdmins) return fakeitem(`Hai Admin *${pushname2}*\nKamu Tidak Akan Ke Kick`)
		nuy.updatePresence(from, Presence.composing)
		if (messagesLink.includes(`${prefix}izinadmin`)) return fakeitem(`*「 DITERIMA 」*\n\n${prefix}izinadmin`)
