...
async function starts() {
	...
	nuy.on('chat-update', async (caa) => {
		try {
            ...
            const ftroli ={key: {fromMe: false, participant: "0@s.whatsapp.net", remoteJid: "0@s.whatsapp.net"}, "message": { "orderMessage": { "itemCount": 0, "status": 200, "thumbnail": fs.readFileSync('pinky.jpg'), "surface": 200, "message": name, "orderTitle": name, "sellerJid": "0@s.whatsapp.net"}}}
...
┬𖧹
╰────────𖧹`
           nuy.sendMessage(from, grupmenu, text, {quoted: ftroli})
           break
           case 'funmenu':
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return fakeitem(mess.only.userB)
...
┬𖧹
╰────────𖧹`
           nuy.sendMessage(from, menuow, text, {quoted: ftroli})
           break
           case 'makermenu':
                if (isBanned) return reply(mess.only.benned)
                if (!isUser) return fakeitem(mess.only.userB)
...
┬𖧹
╰────────𖧹`
           nuy.sendMessage(from, menumaker, text, {quoted: ftroli})
           break
           case 'rules':
           nuy.sendMessage(from, rules(prefix), text, {quoted: ftroli})
    	   break
           case 'return':
				case '>':
				if (!isOwner) return fakeitem(mess.only.ownerB)
...
// makermenu
            ...
            case 'shadow2':
                ...
                case 'carvedwood':
                    ...
                    nuy.sendMessage(from, buffer, image, {caption: 'Nih kak...', quoted: caa})
                    break
            case 'wetglass':
                case 'multicolor3d':
                case 'watercolor':
